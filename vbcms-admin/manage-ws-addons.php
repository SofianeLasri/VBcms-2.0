<?php
$suscribedAddons = $bdd->query("SELECT * FROM `vbcms-wsSuscribedAddons`")->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html>
<head>
						...
						if ($isModule == "module.json") {
							...
							$response = $bdd->prepare("SELECT * FROM `vbcms-modules` WHERE workshopId=?"); // Je récupère l'id du dossier parent
								...
								if (isset($authorDetail["error"]) && $authorDetail["error"] == 404) {
									...
									$authorDetail["link"] = "https://vbcms.net/workshop/profile/".$authorDetail["id"]."-".$authorDetail["slug"];
								}
