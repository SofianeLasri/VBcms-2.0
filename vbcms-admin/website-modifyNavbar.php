...
<head>
	<meta charset="utf-8">
	<title><?=$websiteName?> | Modifier la navbar</title>
	<?php include 'includes/depedencies.php';?>
	<!-- Jquery sortable lists -->
	<script src="<?=$http?>://<?=$_SERVER['HTTP_HOST']?>/vbcms-admin/js/jquery-sortable-lists-navbarEdit.js"></script>
</head>
<body class="position-static">
	...
	<script type="text/javascript">
		...
		async function loadNavbar(parentId,state){
			await $.get("<?=$http?>://<?=$_SERVER['HTTP_HOST']?>/vbcms-admin/backTasks/?loadClientNavbar=all", function(data) {
				var navbarItems = JSON.parse(data);
				jQuery.each(JSON.parse(data), function(index){
		...
		function saveItem(itemId){
			...
			xhr.open("POST", "<?=$http?>://<?=$_SERVER['HTTP_HOST']?>/vbcms-admin/backTasks", true);
		...
		function deleteItem(itemId, confirm){
			if (!confirm) {
				...
				$.get("<?=$http?>://<?=$_SERVER['HTTP_HOST']?>/vbcms-admin/backTasks/?deleteNavItem="+itemId, function(data) {
					if (data=="") {
						SnackBar({
