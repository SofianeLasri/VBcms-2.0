...
const pageFileName = window.location.pathname.match(/([^\/]*)\/*$/)[1];
if (pageFileName=="gallery.php") {
...
async function getChildDirectory(path, parent, genSidebar){
	...
	if (parent == "url") {
		...
		for (let i=0; i<subfolderList.length;i++){//Pour chaque dossier, faire...
			...
			await $.get(siteUrl+"manager/backTask.php?folderContent="+structureCurent+"&folderOnly",async function(data) {// Affiche uniquement les dossiers
				...
				for (let b=0; b<folders.length;b++){
					...
					if (i+1 == subfolderList.length) {
						$("#galleryContent").append('<div id="c'+id+'" type="folder" path="'+path+'/'+name+'" onclick="getChildDirectory(\''+folders[b]+'\', \''+id+'\', \'yes\')" class="col galleryItem"><div class="galleryItemIcon" style="background:url(\''+cdnUrl+'images/manager/folder.png\')"></div><div id="fileName">'+name+'</div></div>');
					}
		...
		await $.get(siteUrl+"manager/backTask.php?folderContent="+path+"&folderOnly", function(data) {// Affiche uniquement les dossiers
			jQuery.each(JSON.parse(data), function(index, val){ 
				...
				$("#galleryContent").append('<div id="c'+id+'" type="folder" path="'+path+'/'+name+'" unselectable="on" onselectstart="return false;" onmousedown="return false;" onclick="getChildDirectory(\''+val+'\', \''+id+'\', \'yes\')" class="col galleryItem"><div class="galleryItemIcon" style="background:url(\''+cdnUrl+'images/manager/folder.png\')"></div><div id="fileName">'+name+'</div>');
				
			});
		});
	}
	
	await $.get(siteUrl+"manager/backTask.php?folderContent="+path+"&filesOnly", function(data) {// Affiche uniquement les fichiers
		jQuery.each(JSON.parse(data), function(index, val){
			var url = cdnUrl+val;
			var name = val.match(/([^\/]*)\/*$/)[1];
			...
			if (name.endsWith(".png")||name.endsWith(".jpg")||name.endsWith(".gif")) {
				...
				$("#galleryContent").append('<div type="image" id="'+id+'" path="'+path+'/'+name+'" onclick="openVideo(\''+path+'/'+name+'\')" class="col galleryItem" unselectable="on" onselectstart="return false;" onmousedown="return false;"><div class="galleryItemIcon" style="background:url(\''+cdnUrl+'/images/manager/video-camera.png\')"></div><div id="fileName">'+name+'</div>');
			} else {
				$("#galleryContent").append('<div type="file" id="'+id+'" path="'+path+'/'+name+'" onclick="download(\''+path+'/'+name+'\')" class="col galleryItem" unselectable="on" onselectstart="return false;" onmousedown="return false;"><div class="galleryItemIcon" style="background:url(\''+cdnUrl+'/images/manager/document.png\')"></div><div id="fileName">'+name+'</div>');
			}
		});
	});
...
if(mode=="full-gallery"){
	...
	function uploadFile(file) {
	    ...
	    xhr.open("POST", siteUrl+"manager/backTask.php", true);
...
if(mode=="full-gallery"){
	...
	function createFolder(){
		...
		$.get(siteUrl+"manager/backTask.php?createFolder="+encodeURIComponent(path+"/"+folderName), function(data) {
			if (data != "") {
				console.log(data);
			} else {
				document.location.reload();
	...
	$("#saveFileDetails").click(function() {
		...
		$.ajax({
		  url: "backTask.php?updateFileDetails="+encodeURIComponent(JSON.stringify(details)),
		});
	...
	$("#explorerDelete").click(async function() {
		path = $("#fileDetailsDiv").attr("path");
		$.ajax({
		  url: "backTask.php?deleteFileFolder="+encodeURIComponent(path),
		});
	});
	...
	$(".fileDetailsDiv").on('click', "#saveFileRename", function(){
		...
		$.ajax({
		  url: "backTask.php?renameFile="+encodeURIComponent(JSON.stringify(details)),
		});
	});
	...
	async function detailsDiv(action){
		...
		if ($("#"+$("#contextMenu").attr("itemId")).attr("type") == "folder"){
			await $.get(siteUrl+"manager/backTask.php?folderDetails="+path, function(data) {// Affiche uniquement les fichiers
			...
			await $.get(siteUrl+"manager/backTask.php?fileDetails="+path, function(data) {// Affiche uniquement les fichiers
				var details = JSON.parse(data);
				if (action == "rename") {
					$(".detailHeader").html("<h5>Renommer</h5><div class=\"d-flex align-items-center\"><input type=\"text\" class=\"form-control\" id=\"fileRenameValue\" value=\""+details[0]+"\" required=\"\"><button id=\"saveFileRename\" type=\"button\" class=\"btn btn-brown float-sm-right ml-2\"><i class=\"fas fa-save\"></i></button></div>");
				} else {
	...
	function copyMoveFile(path, dest, action){
		...
		$.ajax({
		  url: "backTask.php?copyMoveFile="+encodeURIComponent(JSON.stringify(parametres)),
		});
	...
	$("#explorerDelete").click(function() {
		path = $("#"+$("#contextMenu").attr("itemId")).attr("path");
		$.get(siteUrl+"manager/backTask.php?deleteFileFolder="+encodeURIComponent(path), function(data) {
			if (data != "") {
				console.log(data);
			} else {
				document.location.reload();
	...
	function openFileId(pathId){ // Check le type de fichier que c'est
		$.get(siteUrl+"manager/backTask.php?fileDetails="+pathId, function(data) {
			var details = JSON.parse(data);
			name = details[0];
			if (name.endsWith(".png")||name.endsWith(".jpg")||name.endsWith(".gif")) {
				openViewer(pathId);
	...
	function openViewer(pathId){
		...
		$.get(siteUrl+"manager/backTask.php?fileDetails="+pathId, function(data) {
			...
			$.get(siteUrl+"manager/backTask.php?folderContent="+path+"&filesOnly", function(data) {
