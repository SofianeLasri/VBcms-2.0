...
// Fonctions publiques
require $_SERVER['DOCUMENT_ROOT']."/vbcms-content/modules/vbcms-blog/includes/client-functions.php";
...
if ($type=="client") {
	if ($params[0]=="backTasks"){
		...
		$pageTitle = "";
		$pageToInclude = $_SERVER['DOCUMENT_ROOT']."/vbcms-content/modules/vbcms-blog/client/viewArticle.php";
		$customDescription = "";
		$pageDepedencies = "";
	...
	// Fonction admin		
	require $_SERVER['DOCUMENT_ROOT']."/vbcms-content/modules/vbcms-blog/includes/admin-functions.php";
	...
	if (isset($_GET["updateCategory"]) AND !empty($_GET["updateCategory"])) {
		...
		// Variables nécessaire à création d'une page
		$pageTitle = "Éditeur";
		$pageToInclude = $_SERVER['DOCUMENT_ROOT']."/vbcms-content/modules/vbcms-blog/admin/post-new.php";
		createModulePage($pageTitle, "", $pageDepedencies, $pageToInclude, 0);

	} elseif($params[1]=="posts-list"){
		$pageTitle = "Liste des articles";
		$pageToInclude = $_SERVER['DOCUMENT_ROOT']."/vbcms-content/modules/vbcms-blog/admin/posts-list.php";
		createModulePage($pageTitle, "", $pageDepedencies, $pageToInclude, 0);

	} elseif($params[1]=="categories"){
		$pageTitle = "Catégories";
		$pageToInclude = $_SERVER['DOCUMENT_ROOT']."/vbcms-content/modules/vbcms-blog/admin/categories.php";
		createModulePage($pageTitle, "", $pageDepedencies, $pageToInclude, 0);
