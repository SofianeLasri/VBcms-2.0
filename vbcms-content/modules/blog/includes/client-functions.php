...
function getPost($id, $type, $content){
    ...
    if ($type=="published") {
        ...
        $temp["author"] = $_SESSION['user_username'];
        $temp["slug"] = $post["slug"];
...
function getPostsList($condition, $order){
    ...
    if ($condition=="allPosts") {
        $response=$bdd->query("SELECT * FROM `vbcms-blogPosts`")->fetchAll(PDO::FETCH_ASSOC);
        foreach ($response as $post) {
            array_push($results, getPost($post["id"], "published", false));
        }

        $response=$bdd->query("SELECT * FROM `vbcms-blogDrafts`")->fetchAll(PDO::FETCH_ASSOC);
        foreach ($response as $post) {
