...
<script type="text/javascript">
	...
	if (pageFileName=="/vbcms-admin/filemanager/browse") {
	...
	async function getChildDirectory(path, parent, genSidebar){
		...
		if (parent == "url") {
			...
			for (let i=0; i<subfolderList.length;i++){//Pour chaque dossier, faire...
				...
				await $.get("<?=$websiteUrl?>vbcms-admin/filemanager/backTasks/?folderContent="+structureCurent+"&folderOnly",async function(data) {// Affiche uniquement les dossiers
					...
					for (let b=0; b<folders.length;b++){
						...
						if (i+1 == subfolderList.length) {
							$("#galleryContent").append('<div id="c'+id+'" type="folder" path="'+path+'/'+name+'" onclick="getChildDirectory(\''+folders[b]+'\', \''+id+'\', \'yes\')" class="col galleryItem"><div class="galleryItemIcon" style="background:url(\'<?=$websiteUrl?>vbcms-content/modules/vbcms-filemanager/assets/images/manager/folder.png\')"></div><div id="fileName">'+name+'</div></div>');
						}
			...
			await $.get("<?=$websiteUrl?>vbcms-admin/filemanager/backTasks/?folderContent="+path+"&folderOnly", function(data) {// Affiche uniquement les dossiers
				jQuery.each(JSON.parse(data), function(index, val){ 
					...
					$("#galleryContent").append('<div id="c'+id+'" type="folder" path="'+path+'/'+name+'" unselectable="on" onselectstart="return false;" onmousedown="return false;" onclick="getChildDirectory(\''+val+'\', \''+id+'\', \'yes\')" class="col galleryItem"><div class="galleryItemIcon" style="background:url(\'<?=$websiteUrl?>vbcms-content/modules/vbcms-filemanager/assets/images/manager/folder.png\')"></div><div id="fileName">'+name+'</div>');
					
				});
			});
		}
		
		await $.get("<?=$websiteUrl?>vbcms-admin/filemanager/backTasks/?folderContent="+path+"&filesOnly", function(data) {// Affiche uniquement les fichiers
			jQuery.each(JSON.parse(data), function(index, val){
				var url = cdnUrl+val;
				var name = val.match(/([^\/]*)\/*$/)[1];
				...
				if (name.endsWith(".png")||name.endsWith(".jpg")||name.endsWith(".gif")) {
					...
					$("#galleryContent").append('<div type="image" id="'+id+'" path="'+path+'/'+name+'" onclick="openVideo(\''+path+'/'+name+'\')" class="col galleryItem" unselectable="on" onselectstart="return false;" onmousedown="return false;"><div class="galleryItemIcon" style="background:url(\'<?=$websiteUrl?>vbcms-content/modules/vbcms-filemanager/assets//images/manager/video-camera.png\')"></div><div id="fileName">'+name+'</div>');
				} else {
					$("#galleryContent").append('<div type="file" id="'+id+'" path="'+path+'/'+name+'" onclick="download(\''+path+'/'+name+'\')" class="col galleryItem" unselectable="on" onselectstart="return false;" onmousedown="return false;"><div class="galleryItemIcon" style="background:url(\'<?=$websiteUrl?>vbcms-content/modules/vbcms-filemanager/assets//images/manager/document.png\')"></div><div id="fileName">'+name+'</div>');
				}
			});
		});
	...
	if(mode=="full-gallery"){
		...
		function uploadFile(file) {
		    ...
		    xhr.open("POST", "<?=$websiteUrl?>vbcms-admin/filemanager/backTasks", true);
	...
	if(mode=="full-gallery"){
		...
		function createFolder(){
			...
			$.get("<?=$websiteUrl?>vbcms-admin/filemanager/backTasks/?createFolder="+encodeURIComponent(path+"/"+folderName), function(data) {
				if (data != "") {
					console.log(data);
				} else {
					document.location.reload();
		...
		$("#saveFileDetails").click(function() {
			...
			$.ajax({
			  url: "<?=$websiteUrl?>vbcms-admin/filemanager/backTasks/?updateFileDetails="+encodeURIComponent(JSON.stringify(details)),
			});
		...
		$("#explorerDelete").click(async function() {
			path = $("#fileDetailsDiv").attr("path");
			$.ajax({
			  url: "<?=$websiteUrl?>vbcms-admin/filemanager/backTasks/?deleteFileFolder="+encodeURIComponent(path),
			});
		});
		...
		$(".fileDetailsDiv").on('click', "#saveFileRename", function(){
			...
			$.ajax({
			  url: "<?=$websiteUrl?>vbcms-admin/filemanager/backTasks/?renameFile="+encodeURIComponent(JSON.stringify(details)),
			});
		});
		...
		async function detailsDiv(action){
			...
			if ($("#"+$("#contextMenu").attr("itemId")).attr("type") == "folder"){
				await $.get("<?=$websiteUrl?>vbcms-admin/filemanager/backTasks/?folderDetails="+path, function(data) {// Affiche uniquement les fichiers
				...
				await $.get("<?=$websiteUrl?>vbcms-admin/filemanager/backTasks/?fileDetails="+path, function(data) {// Affiche uniquement les fichiers
					var details = JSON.parse(data);
					if (action == "rename") {
						$(".detailHeader").html("<h5>Renommer</h5><div class=\"d-flex align-items-center\"><input type=\"text\" class=\"form-control\" id=\"fileRenameValue\" value=\""+details[0]+"\" required=\"\"><button id=\"saveFileRename\" type=\"button\" class=\"btn btn-brown float-sm-right ml-2\"><i class=\"fas fa-save\"></i></button></div>");
					} else {
		...
		function copyMoveFile(path, dest, action){
			...
			$.ajax({
			  url: "<?=$websiteUrl?>vbcms-admin/filemanager/backTasks/?copyMoveFile="+encodeURIComponent(JSON.stringify(parametres)),
			});
		...
		$("#explorerDelete").click(function() {
			path = $("#"+$("#contextMenu").attr("itemId")).attr("path");
			$.get("<?=$websiteUrl?>vbcms-admin/filemanager/backTasks/?deleteFileFolder="+encodeURIComponent(path), function(data) {
				if (data != "") {
					console.log(data);
				} else {
					document.location.reload();
		...
		function openFileId(pathId){ // Check le type de fichier que c'est
			$.get("<?=$websiteUrl?>vbcms-admin/filemanager/backTasks/?fileDetails="+pathId, function(data) {
				var details = JSON.parse(data);
				name = details[0];
				if (name.endsWith(".png")||name.endsWith(".jpg")||name.endsWith(".gif")) {
					openViewer(pathId);
		...
		function openViewer(pathId){
			...
			$.get("<?=$websiteUrl?>vbcms-admin/filemanager/backTasks/?fileDetails="+pathId, function(data) {
				...
				$.get("<?=$websiteUrl?>vbcms-admin/filemanager/backTasks/?folderContent="+path+"&filesOnly", function(data) {
